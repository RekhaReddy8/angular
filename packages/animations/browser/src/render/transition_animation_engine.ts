  ...
  removeNode(element: any, context: any, doNotRecurse?: boolean): void {
    ...
    if (listeners) {
      ...
      listeners.forEach(listener => {
        ...
        const fromState = engine.statesByElement.get(element)[triggerName] || DEFAULT_STATE_VALUE;
...
export class TransitionAnimationEngine {
  ...
  private _flushAnimations(): TransitionAnimationPlayer[] {
    ...
    this.queuedRemovals.forEach((fn, element) => {
      ...
      if (players) {
        ...
        let elementPlayers: AnimationPlayer[] = null;
  ...
  private _beforeAnimationBuild(
    ...
    const targetNameSpaceId: string = instruction.isRemovalTransition ? undefined : namespaceId;
    const targetTriggerName: string = instruction.isRemovalTransition ? undefined : triggerName;
